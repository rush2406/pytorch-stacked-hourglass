...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.depth1 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,dilation=2,padding=2, bias=True)
        self.depth2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride, dilation=3 ,padding=3, bias=True)
    ...
    def forward(self, x):
        ...
        out = self.bn2(out)
        inter = self.relu(out)
        out1 = self.conv2(inter)
        #print(out1.shape)
        out2 = self.depth1(inter)
        #print(out2.shape)
        out3 = self.depth2(inter)
        #print(out3.shape)
        out = out1+out2+out3
        out = self.bn3(out)
        out = self.relu(out)
...
class Hourglass(nn.Module):
    ...
    def _make_hour_glass(self, block, num_blocks, planes, depth):
        ...
        for i in range(depth):
            res = []
            for j in range(3):#change number of bottlenecks
    ...
    def __init__(self, block, num_stacks=2, num_blocks=4, num_classes=16):
        ...
        for i in range(num_stacks):
            if i<3:
                        ...
                        d = 4
            hg.append(Hourglass(block, num_blocks, self.num_feats, 4))#4
...
def hg8(pretrained=False, progress=True, num_blocks=1, num_classes=16):
    print('here')
    return _hg('hg8', pretrained, progress, num_stacks=6, num_blocks=num_blocks,num_classes=num_classes)#change stacks
